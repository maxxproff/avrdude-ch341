...
// ----------------------------------------------------------------------
/* Prototypes */
static void ch341a_bitbang_setup(PROGRAMMER * pgm);
static void ch341a_bitbang_teardown(PROGRAMMER * pgm);
static int  ch341a_bitbang_open(PROGRAMMER * pgm, char * port);
static void ch341a_bitbang_close(PROGRAMMER * pgm);
static int ch341a_bitbang_initialize(PROGRAMMER * pgm, AVRPART * p);
static int ch341a_bitbang_spi_cmd(PROGRAMMER * pgm, const unsigned char *cmd, unsigned char *res);
static int ch341a_bitbang_spi(PROGRAMMER * pgm, const unsigned char *in, unsigned char *out, int size);
static int ch341a_bitbang_spi_program_enable(PROGRAMMER * pgm, AVRPART * p);
static int ch341a_bitbang_spi_chip_erase(PROGRAMMER * pgm, AVRPART * p);
//static int ch341a_bitbang_spi_transfer(PROGRAMMER * pgm, const unsigned char *cmd, unsigned char *res);
// dummy functions
static void ch341a_bitbang_disable(PROGRAMMER * pgm);
static void ch341a_bitbang_enable(PROGRAMMER * pgm);
static void ch341a_bitbang_display(PROGRAMMER * pgm, const char * p);

BOOL	MegaSpiOutInData(PROGRAMMER * pgm, ULONG OutLen, PVOID DataBuf, PVOID DataBufOut );
...
static int CH341USBTransfer_bitbangPart(PROGRAMMER * pgm, enum libusb_endpoint_direction dir, unsigned char *buff, unsigned int size) {
...
static bool CH341USBTransfer_bitbang(PROGRAMMER * pgm, enum libusb_endpoint_direction dir, unsigned char *buff, unsigned int size)
{
    int pos = 0, bytestransferred;
    while (size) {
        bytestransferred = CH341USBTransfer_bitbangPart(pgm, dir, buff + pos, size);
 ...
 *  D7/22   SO/2    (DIN)
 */
bool CH341ChipSelect_bitbang(PROGRAMMER * pgm, unsigned int cs, bool enable) {
    unsigned char res[4];
    unsigned char cmd[4];
    memset(cmd, 0, sizeof(cmd));
    memset(res, 0, sizeof(res));
    avrdude_message(MSG_DEBUG, "%s: ch341a_bitbang_ChipSelect()\n", progname);
    if (cs > 2) {
    ...
    return CH341USBTransfer_bitbangPart(pgm, LIBUSB_ENDPOINT_OUT, cmd, 3);
}
...
static int ch341a_bitbang_open(PROGRAMMER * pgm, char * port) {
    ...
    avrdude_message(MSG_DEBUG, "%s: ch341a_bitbang_open(\"%s\")\n", progname, port);
...
static void ch341a_bitbang_close(PROGRAMMER * pgm) {
    avrdude_message(MSG_DEBUG, "%s: ch341a_bitbang_close()\n", progname);
    CH341ChipSelect_bitbang(pgm, 0,false);
...
static int ch341a_bitbang_initialize(PROGRAMMER * pgm, AVRPART * p) {
    avrdude_message(MSG_DEBUG, "%s: ch341a_bitbang_initialize()\n", progname);
    CH341SetStream_bitbang( pgm, 0x81 );
    MegaSpiEnable( pgm );
    CH341ChipSelect_bitbang(pgm, 0,false);
    usleep(20 * 1000);
    CH341ChipSelect_bitbang(pgm, 0,true);
...
/* static int ch341a_bitbang_spi_transfer(PROGRAMMER * pgm, const unsigned char *cmd, unsigned char *res) {
    ...
    avrdude_message(MSG_DEBUG, "%s: ch341a_bitbang_spi_transfer(0x%02x, 0x%02x, 0x%02x, 0x%02x)%s",
...
static int ch341a_bitbang_spi(PROGRAMMER * pgm, const unsigned char *in, unsigned char *out, int size) {
    ...
    if (!MegaSpiOutInData(pgm, (ULONG)size, (PVOID)in, out))
    {
...
static int ch341a_bitbang_spi_program_enable(PROGRAMMER * pgm, AVRPART * p) {
    ...
    cmd[0] = 0;
    avrdude_message(MSG_DEBUG, "%s: ch341a_bitbang_program_enable() %s\n",
...
static int  ch341a_bitbang_spi_chip_erase(struct programmer_t * pgm, AVRPART * p) {
    unsigned char cmd[4];
    unsigned char res[4];
...
void ch341a_bitbang_initpgm(PROGRAMMER * pgm) {
    strcpy(pgm->type, "ch341a_bitbang");
    /*
    * mandatory functions
    */
    pgm->initialize     = ch341a_bitbang_initialize;
    pgm->display        = ch341a_bitbang_display;
    pgm->enable         = ch341a_bitbang_enable;
    pgm->disable        = ch341a_bitbang_disable;
    pgm->program_enable = ch341a_bitbang_spi_program_enable;
    pgm->chip_erase     = ch341a_bitbang_spi_chip_erase;
    pgm->cmd            = ch341a_bitbang_spi_cmd;
    pgm->spi            = ch341a_bitbang_spi;
    pgm->open           = ch341a_bitbang_open;
    pgm->close          = ch341a_bitbang_close;
    ...
    */
    ...
    //pgm->paged_write    = ch341a_bitbang_spi_paged_write;
    //pgm->paged_load     = ch341a_bitbang_spi_paged_load;
    pgm->setup          = ch341a_bitbang_setup;
    pgm->teardown       = ch341a_bitbang_teardown;
...
static void ch341a_bitbang_setup(PROGRAMMER * pgm) {
    if ((pgm->cookie = malloc(sizeof(struct pdata))) == 0) {
        avrdude_message(MSG_INFO, "%s: ch341a_bitbang_setup(): Out of memory allocating private data\n",
                        progname);
        exit(1);
    }
...
static int ch341a_bitbang_nousb_open (struct programmer_t *pgm, char * name) {
...
void ch341a_bitbang_initpgm(PROGRAMMER * pgm) {
    strcpy(pgm->type, "ch341a_bitbang");
    pgm->open = ch341a_bitbang_nousb_open;
}
...
//UCHAR  zzzzBuffer[ 4 ];
    ...
    // 202334066 on? + 6.95us - 5.85us
        ...
        //avrdude_message(MSG_DEBUG, "MegaSpiOutInData: before CH341_write_read OutLen = %d \n", (int)OutLen);
        ...
        //avrdude_message(MSG_DEBUG, "MegaSpiOutInData: after CH341_write_read OutLen = %d \n", (int)OutLen);
        ...
        for ( i = 0; i != OutLen; i++ )
        {
            ...
            *(PUCHAR)DataBufOut = c;
            //((PUCHAR)DataBuf)++;
            //avrdude_message(MSG_DEBUG, "MegaSpiOutInData: DataBufOut[%x] = %x \n", (int)i, (int)c);
            DataBufOut++;
        } 
        //for( zz = 0; zz != j; zz++ )
            //{
...
ULONG   i;
    ...
    if (!CH341USBTransfer_bitbang(pgm, LIBUSB_ENDPOINT_OUT, mBuffer, 5))
    {
